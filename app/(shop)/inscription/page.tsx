...
export default function InscriptionPage() {
  ...
  async function onSubmit(values: RegisterFormValues) {
    try {
      ...
      toast.success("Compte créé — bienvenue sur RHO !");
      router.push("/compte");
    } catch (error) {
  ...
  return (
    <div className="mx-auto flex min-h-[80vh] max-w-md flex-col justify-center px-4 pt-20 sm:px-6">
      ...
      <p className="mt-1 text-sm text-text-muted">
        Rejoignez RHO pour suivre vos commandes et profiter du programme de fidélité.
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            />
            {errors.firstName && (
              <p className="mt-1 text-xs text-rho-danger">{errors.firstName.message}</p>
            )}
          </div>
          <div>
            ...
            />
            {errors.lastName && (
              <p className="mt-1 text-xs text-rho-danger">{errors.lastName.message}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          {errors.confirmPassword && (
            <p className="mt-1 text-xs text-rho-danger">{errors.confirmPassword.message}</p>
          )}
        </div>
      ...
      <p className="mt-6 text-center text-sm text-text-muted">
        Déjà un compte ?{" "}
        <Link href="/connexion" className="font-medium text-rho-blue-600 hover:underline">
          Se connecter
        </Link>
      </p>
    </div>
